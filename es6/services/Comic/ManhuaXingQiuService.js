...
export default class ManhuaXingQiuService extends Base {
    ...
    static async get_base_info(ctx, source_id) {
        Log.ctxInfo(ctx, `ManhuaXingQiuService 开始拉取 source_id ${source_id} 基本信息`)
        ...
        await TimeTool.delay_rand_ms(500, 1500) // 限速
        Log.ctxInfo(ctx, `继续`)
        ...
        return fetch(target_url, options)
            ...
            .then(html => {
                ...
                let intro = $("#comic-description").text()
    ...
    static async get_chapter_list(ctx, source_id) {
        let _this = this
        Log.ctxInfo(ctx, `ManhuaXingQiuService 开始拉取 source_id ${source_id} 列表信息`)
        let target_url = `${BASE_HREF}/${source_id}/`
        let sequence = 0
        let chapter_list = []
        let options = {
    ...
    static getLink(path) {
        return ManhuaXingQiuService.get_base_href() + path
    }
    ...
    static async get_image_list(ctx, target_url) {
        let image_list = [];
        Log.ctxInfo(ctx, `ManhuaXingQiuService 开始拉取 ${target_url} 图片列表`)
